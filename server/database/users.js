...
module.exports = {
  ...
  getUsers: (username, password, callback) => {
    const queryString = `SELECT users.firstName, users.lastName, users.email, users.username,
    (SELECT JSON_ARRAYAGG(JSON_OBJECT("title", title, "body", body, "createdAt", createdAt)) from entries WHERE users.id = userId) AS "entries" FROM users WHERE username = "${username}" AND password = "${password}";`;
